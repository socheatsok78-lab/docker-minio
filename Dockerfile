...
FROM scratch AS mc
ARG TARGETARCH
ADD https://dl.min.io/client/mc/release/linux-${TARGETARCH}/mc /mc
ADD https://dl.min.io/client/mc/release/linux-${TARGETARCH}/mc.minisig /mc.minisig
ADD https://dl.min.io/client/mc/release/linux-${TARGETARCH}/mc.sha256sum /mc.sha256sum
...
COPY --from=mc /mc /go/bin/mc
COPY --from=mc /mc.minisig /go/bin/mc.minisig
COPY --from=mc /mc.sha256sum /go/bin/mc.sha256sum
...
RUN <<EOT
    ...
    GOLDFLAGS="${GOLDFLAGS} -X ${GIT_IMPORT}.ReleaseTag=${MINIO_VERSION}"
    GOLDFLAGS="${GOLDFLAGS} -X ${GIT_IMPORT}.CommitID=${GIT_COMMIT}"
    GOLDFLAGS="${GOLDFLAGS} -X ${GIT_IMPORT}.ShortCommitID=${GIT_COMMIT:0:12}${GIT_DIRTY}"
...
COPY --from=mc /go/bin/mc* /usr/bin/
